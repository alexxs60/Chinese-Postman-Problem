...
一个基于 C++ 的中国邮路问题（Chinese Postman Problem）完整求解程序，能够在连通的无向加权图中找到遍历每条边至少一次的最短闭合回路（邮递员最优路线）。
项目亮点

完整图论算法实现（Floyd-Warshall + 最小完美匹配 + Hierholzer 欧拉回路）
...
- 输入灵活：支持键盘手动输入 + 文件批量输入
- 性能统计：记录算法运行时间，便于测试与分析
>>>>>>> 50bc407a2ab5772aa9fbe5b3e61559da70f96f55
...
Bashg++ -std=c++11 main.cpp -o cpp
# 或带优化
g++ -std=c++11 -O2 main.cpp -o cpp

运行程序

Bash./cpp                # Linux / macOS
cpp.exe              # Windows
...
运行效果截图
（建议添加 2-3 张截图，放在 screenshots/ 文件夹）
菜单界面
输入图数据
输出最佳路线
注意事项
...
节点编号从 0 开始
图必须连通，否则程序会提示无法求解
奇度节点数量必须为偶数（图论性质），否则匹配失败
奇度节点过多（>12 个）时递归匹配较慢，适合课程设计小规模数据
权重建议为正整数（负权未测试）
输出路线可能因 multiset 邻接选择顺序不同而有多种等价解，但总路程固定正确

学习收获

深入掌握图论核心算法与中国邮路问题完整求解流程
理解奇度节点匹配、重复边添加与欧拉图转化原理
培养高质量代码习惯（清晰结构、异常处理、性能考虑）
